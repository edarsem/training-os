...
def get_route_details_tool(db: DBSession, *, route_id: int) -> dict[str, Any]:
    ...
    # Compute the actual-performance comparison first: when it exists, its per-bracket and per-km
    # blocks subsume the planning gradient distribution / per-km elevation profile, so we tell
    # build_route_text_summary to drop those (has_actuals) and avoid paying for duplicate tokens.
    comparison = None
    ...
    lines = [build_route_text_summary(
        route, markers, histogram, track=track, metrics=metrics, has_actuals=comparison is not None
    )]
    if comparison:
        lines.append(
